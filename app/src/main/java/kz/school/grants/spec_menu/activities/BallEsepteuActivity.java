...
public class BallEsepteuActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    @BindView(R.id.entBall) EditText entBall;
    @BindView(R.id.llProgressBar) View llProgressBar;
    @BindView(R.id.calcBtn)  Button calcBtn;
    @BindView(R.id.auilGroup) CheckBox auilGroup;
    @BindView(R.id.kazGroup) RadioButton kazGroup;
    @BindView(R.id.rusGroup) RadioButton rusGroup;
...
//                        Toast.makeText(BallEsepteuActivity.this, profCode[0].trim()+"!!!", Toast.LENGTH_SHORT).show();
    ...
    public void onClick(View view) {
        ...
        Cursor cursor = sqdb.rawQuery(
                "SELECT * FROM " + TABLE_GRANTS + " where " +COLUMN_SUBJECTS_ID + " = ? AND " + columnSearchMin + " <= ?",
